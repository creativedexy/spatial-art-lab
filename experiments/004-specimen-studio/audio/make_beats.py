...
class Track:
    def __init__(self, bpm, bars=16):
        ...
        self.mix = np.zeros(self.n); self.duck = np.ones(self.n); self.ev = {'kick': [], 'snare': [], 'hat': []}

    def at(self, bar, step):                                      # sample index of a 16th step
        return int(round((bar * 16 + step) * self.step * SR))

    def put(self, s, i, g=1.0, tag=None):
        if i >= self.n: return
        if tag: self.ev[tag].append(round(i / SR, 4))
        e = min(self.n, i + len(s)); self.mix[i:e] += s[:e - i] * g

    def pattern(self, sound, steps, g=1.0, bars=None, swing=0.0, tag=None):
        for b in (bars if bars is not None else range(self.bars)):
            for st in steps:
                sh = int(swing * self.step * SR) if st % 2 else 0
                self.put(sound, self.at(b, st) + sh, g, tag)
    ...
    def write(self, name):
        ...
        side = {'bpm': self.bpm, 'offset': 0.0, 'duration': round(self.n / SR, 4), 'source': 'exact', **{k: sorted(v) for k, v in self.ev.items()}}
        (HERE / name).with_suffix('.beats.json').write_text(__import__('json').dumps(side))   # exact beat map for the studio
...
def pulse():                                                      # techno 128: four-on-the-floor, rumble, acid line opening up
    ...
    tr.pattern(K, [0, 4, 8, 12], 1.0, tag='kick'); tr.sidechain([0, 4, 8, 12], 0.75, 0.2)
    tr.pattern(hat(0.16, 6000), [2, 6, 10, 14], 0.35, tag='hat'); tr.pattern(hat(0.03, 9000), range(16), 0.12, bars=range(4, 16))
    tr.pattern(clap(), [4, 12], 0.5, bars=range(4, 16), tag='snare')
...
def wobble():                                                     # halftime 140: kick, slam snare, distorted wobble bass
    ...
    tr.pattern(K, [0, 10], 1.0, bars=range(2, 16), tag='kick'); tr.pattern(S, [8], 1.0, bars=range(2, 16), tag='snare'); tr.sidechain([0, 10], 0.6, 0.15, bars=range(2, 16))
    tr.pattern(hat(0.04, 8000), [2, 6, 10, 14], 0.2, tag='hat'); tr.pattern(hat(0.02, 9000), [3, 7, 11, 13, 15], 0.1, bars=range(8, 16), tag='hat')
    roots = [29, 29, 32, 27]
...
def breakneck():                                                  # drum & bass 172: broken beat, ghost snares, reese bass
    ...
    tr.pattern(K, [0, 10], 1.0, tag='kick'); tr.pattern(S, [4, 12], 1.0, tag='snare'); tr.pattern(G, [7, 9, 15], 0.28, bars=range(2, 16))
    tr.pattern(K, [2], 0.7, bars=range(1, 16, 2), tag='kick'); tr.pattern(hat(0.03, 8000), range(0, 16, 2), 0.22, tag='hat'); tr.pattern(hat(0.015, 10000), range(1, 16, 2), 0.1)
...
def night808():                                                   # trap 140 (halftime feel): rolling hats, sliding 808s, hard snare
    ...
    tr.pattern(S, [8], 1.0, tag='snare'); tr.pattern(clap(), [8], 0.5)
    for b in range(tr.bars):                                      # hats: 8ths with triplet and 32nd rolls
        for st in range(0, 16, 2): tr.put(hat(0.03, 8500), tr.at(b, st), 0.25, 'hat')
    ...
    for b in range(tr.bars):
        for st, m, dur in hits:
            ...
            tr.put(kick(0.12, 200, 60, 0.02, 1.2), i, 0.8, 'kick')
...
def hammer():                                                     # industrial hard 150: distorted kick tails, clangs, noise bursts
    ...
    tr.pattern(K, [0, 4, 8, 12], 1.0, bars=range(1, 16), tag='kick'); tr.sidechain([0, 4, 8, 12], 0.5, 0.2, bars=range(1, 16))
    tr.pattern(clang(0.5, 330), [6], 0.45, bars=range(2, 16, 2), tag='snare'); tr.pattern(clang(0.7, 247), [14], 0.4, bars=range(3, 16, 2), tag='snare')
    burst = hp(rng.uniform(-1, 1, int(0.12 * SR)), 900) * env(int(0.12 * SR), 0.001, 0.12)
    tr.pattern(burst, [3, 11], 0.4, bars=range(4, 16), tag='snare'); tr.pattern(hat(0.05, 5000), [2, 6, 10, 14], 0.3, tag='hat')
...
def braam():                                                      # cinematic heavy 90: braam hits, sub drops, war toms, riser
    ...
    for b in range(0, tr.bars, 2):
        ...
        tr.put(x * 0.35 + sub * 0.9, i, 1.0, 'kick')
    tr.pattern(tom(0.8, 80), [0, 3, 6], 0.7, bars=range(4, 16), tag='kick'); tr.pattern(tom(0.6, 110), [10, 11, 13], 0.5, bars=range(8, 16), tag='hat')
    tr.pattern(snare(0.5, 150, 1.2, 1200), [8], 0.8, bars=range(8, 16), tag='snare')
