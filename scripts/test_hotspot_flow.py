...
def main():
    ...
    with sync_playwright() as pw:
        ...
        page.goto(url)
        page.wait_for_function("window.__terrainReady === true", timeout=120_000)
        ...
        page.wait_for_timeout(2600)              # mid-descent: the clip is up
        if shots:
            page.screenshot(path=str(shots / "02-descending.png"))
        played = page.evaluate(
            "() => { const v = document.querySelector('.descent-clip');"
            " return v ? { t: v.currentTime, w: v.videoWidth, hidden: v.hidden } : null; }")
        check("the clip is playing, not the fallback", bool(played) and played["t"] > 0.2,
              f"currentTime={played and round(played['t'], 2)}s, {played and played['w']}px wide")

        page.wait_for_selector("#panel:not([hidden])", timeout=30_000)
        ...
        page.wait_for_selector("#panel", state="hidden", timeout=30_000)
        page.wait_for_timeout(600)
        if shots:
            page.screenshot(path=str(shots / "04-returned.png"))
        check("every hotspot is back on the map",
              page.locator(".hotspot:visible").count() == 3)
